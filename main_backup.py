...
class StrategyAgentSystem:
    ...
    def convert_to_pdf(self, markdown_file, output_dir):
        """Convert markdown to PDF (if WeasyPrint is available)"""
    ...
    def run_parallel_analysis(self):
        ...
        # Create tasks using existing task methods
        business_task = self.tasks.business_intelligence_task(
            business_analyst, self.business_info, self.business_goals
        )
        
        competitor_task = self.tasks.competitor_analysis_task(
            competitor_analyst, self.business_info
        )
        
        # For now, use business intelligence task as marketing evaluation
        evaluation_task = self.tasks.business_intelligence_task(
            marketing_evaluator, self.current_marketing, self.budget_info
        )
        
        # Run sequential crew (more reliable than hierarchical)
        parallel_crew = Crew(
    ...
    def run_strategy_composition(self, analysis_results):
        ...
        composition_task = self.tasks.marketing_strategy_development_task(
            strategy_composer,
    ...
    def run_qa_supervision(self, strategy_result):
        ...
        # For QA, use the marketing strategy development task
        qa_task = self.tasks.marketing_strategy_development_task(
            qa_supervisor,
            self.shared_memory.retrieve("business_analysis"),
            strategy_result
        )
